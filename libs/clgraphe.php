...
class clgraphe  {
	...
	function valeurbnmgraphe($TBL,$COLONE1,$DATEJOUR1,$DATEJOUR2,$moisbnm) 
	{
		$this->mysqlconnectx();
		$sql="select $COLONE1 from $TBL where  $COLONE1 BETWEEN '$DATEJOUR1' AND '$DATEJOUR2' and mois=$moisbnm";
		$requete = @mysql_query($sql) or die($sql."<br>".mysql_error());
		$OP=mysql_num_rows($requete);
		mysql_free_result($requete);
		return $OP;
	}
	function bnmgraphe($x,$y,$TITRE,$TBL,$COL,$COLONE,$VALEUR1,$moisbnm) 
	{
	...
	for ($i = 9; $i >= 0; $i-= 1){
	$serie1->addPoint(new Point($DATE-$i,$this->valeurbnmgraphe($TBL,$COL,($DATE-$i),($DATE-$i),$moisbnm)));	
	}
